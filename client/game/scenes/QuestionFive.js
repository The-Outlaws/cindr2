...
export default class QuestionFive extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(
      ...
      `The cat’s kitten has climbed a tree and can’t get down! You:`,
      fontStyleQuestion
    );
    this.answerA = this.add.text(
      3.62 * this.bg.displayWidth / 4,
      2.8 * this.bg.displayHeight / 4,
      'Climb the tree and help him down, braving a few scratches.',
      fontStyleAnswer
    );
    this.answerB = this.add.text(
      1.65 * this.bg.displayWidth / 4,
      1.2 * this.bg.displayHeight / 4,
      'Get someone else to do it - you’re terrified of heights.',
      fontStyleAnswer
    );
